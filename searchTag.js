...
let [execPath, arvishInput, option] = process.argv.slice(1)

arvishInput = replaceAll(handleInput(arvishInput), '\\"', '"')
...
(async function () {
  arvish.output(await api.listTags(
    {
      callback: searchTag
    }
  ))

  LogManager.write(`ent ${handleInput(arvish.input)}`)
})()
