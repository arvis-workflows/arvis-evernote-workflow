...
(async function () {
  ...
  const getSubtitle = async (type, searchedNotes, selectedNote) => {
    switch (type) {
      ...
      case 'tags': {
        return await api.getNoteTagNames(
  ...
  const getResult = async (searchedNotes) => {
    ...
    const result = await Promise.all(
      _.map(searchedNotes, async (note) => {
        ...
        const quicklookurl =
          process.env.initialCaching === 'true'
  ...
  (async function () {
    arvish.output(
      await api.findNotesMetadata(filter, 0, config.search_max_count, spec, {
        callback: searchNote
      })
    )

    LogManager.write(`${command} ${handleInput(arvish.input)}`)
  })()
})()
